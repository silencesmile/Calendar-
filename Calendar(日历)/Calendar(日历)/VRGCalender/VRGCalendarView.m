...
#define kVRGCalendarViewDayWidth (kVRGCalendarViewWidth - 2 * margin01)/7
#define kVRGCalendarViewDayHeight 40
// alertHeight
#define BQAlertViewCellHeight 40
// 底部view弹出
...
-(void)markDates:(NSArray *)dates {
    ...
    for (int i = 0; i<[dates count]; i++) {
        // day的标记--下划线
        [colors addObject:[UIColor colorWithHexString:@"0xff0000"]];
    }
...
//    labelCurrentMonth.backgroundColor = [UIColor blueColor];
    // 月份
...
//    NSLog(@"currentMonth month = %i day = %i, todaydate day = %i",[currentMonth month],[currentMonth day],[todayDate month]);
    ...
    for (int i=0; i<numBlocks; i++) {
        ...
        if (selectedDate && i==selectedDateBlock) {
            ...
            CGContextAddRect(context, rectangleGrid);
            // 设置手点颜色 浅红较好
            CGContextSetFillColorWithColor(context, [UIColor colorWithHexString:@"0xFF7985"].CGColor);
            CGContextFillPath(context);
        ...
        // 文字的颜色
        [date drawInRect:CGRectMake(targetX+2, targetY+10, kVRGCalendarViewDayWidth, kVRGCalendarViewDayHeight) withFont:[UIFont fontWithName:@"HelveticaNeue-Bold" size:15] lineBreakMode:UILineBreakModeClip alignment:UITextAlignmentCenter];
    }
...
-(id)init {
    ...
    if (self) {
        ...
        // 标题月份加年颜色
        labelCurrentMonth.textColor = [UIColor colorWithHexString:@"0xff0000"];
